...
static int create_async_eqs(struct mlx5_core_dev *dev)
{
	 ...
	 * leave cmd_eq in polling mode -- mirrors vfio-pci-mlx5's
	 * "VF cmd interface is not host-driven post-restore" property and
	 * removes the empirical trigger of the post-LOAD slot-0 ghost EQE
	 * / lost-completion failure mode. cmd_eq is still created above so
	 * FW has somewhere to drop cmd-completion EQEs if it emits any;
	 * we just never register a notifier or read from it. All cmds on
	 * this mdev (including the async_eq / pages_eq CREATE_EQ that
	 * follow) complete via lay->status_own polling on the cmd ring.
	 * destroy_async_eqs() has the matching skip on teardown.
	 */
...
err2:
	/*
	 * vfmig: only switch back to polling mode if create_async_eqs()
	 * actually switched into events mode above. With
	 * vfmig_skip_cmd_use_events the cmd-comp notifier was never
	 * registered, so the matching unregister in mlx5_cmd_use_polling()
	 * would walk a list the notifier is not on.
	 */
	if (!dev->cmd.vfmig_skip_cmd_use_events)
